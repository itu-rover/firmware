...
int checksum(const char *s) {
    ...
    if (*s++ == '$') while (*s != '*' && *s != '\0') c ^= *s++;

    return c;
}
char checksum_check(const char *s) {
    int c = 0;
...
int comma_number(const char *str) {
    return 1;
}
...
TM_GPS_Result_t TM_GPS_Update(TM_GPS_t* GPS_Data,char buff, uint8_t length) {
	/* Check for data in USART */
	uint8_t i;
	for(i=0;i<length;i++){
	//if (!GPS_USART_BUFFER_EMPTY) {
		//while (!GPS_USART_BUFFER_EMPTY) {
			TM_GPS_INT_Do(GPS_Data, (char)buff);
